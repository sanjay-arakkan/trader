...
export function JournalTable() {
  ...
  const days = React.useMemo(() => {
    ...
    return eachDayOfInterval({ start, end }).filter((day) => {
      ...
      if (config.startDate && startOfDay(day) < startOfDay(config.startDate))
        return false;
      return true;
    });
  ...
  const calculateValues = (day: Date, entry: JournalEntry | undefined) => {
    ...
    if (config.startDate && config.initialCapital > 0) {
      ...
      if (!isAfter(current, target)) {
        ...
        projectedCapital =
          config.initialCapital * Math.pow(1.01, tradingDayIndex);
      }
    }
    ...
    const profitPercent =
      manualCapital > 0 ? (netProfit / manualCapital) * 100 : 0;
    ...
    return {
      ...
      maxBrokerage:
        maxBrokerage > 0 ? formatCurrency(maxBrokerage.toFixed(0)) : "",
      brokPercent:
        brokerage === 0
          ? "0.00%"
          : profit !== 0
            ? brokPercent.toFixed(2) + "%"
            : "",
  ...
  const toggleNoteEdit = async (weekKey: string, weekStart: Date) => {
    if (editingNote === weekKey) {
      ...
      if (hasChanges) {
        try {
          await journalService.saveWeeklyNote(
            weekKey,
            weekStart,
            weekNotes[weekKey] || "",
          );
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0">
          <div className="flex items-center gap-4">
            <Button
              ...
              disabled={
                config.startDate
                  ? startOfMonth(currentMonth) <= startOfMonth(config.startDate)
                  : false
              }
            >
                      ...
                      >
                        ...
                        <TableCell className="h-12 py-1">
                          {isEditing ? (
                            <Select
                              ...
                              onValueChange={(val) =>
                                handleInputChange(dateStr, "status", val)
                              }
                            >
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="target_failed">
                                  Target Failed
                                </SelectItem>
                                <SelectItem value="market_holiday">
                                  Market Holiday
                                </SelectItem>
                                <SelectItem value="special_occasion">
                                  Special Occasion
                                </SelectItem>
                                <SelectItem value="no_trade">
                                  No Trade
                                </SelectItem>
                              </SelectContent>
                            </Select>
                        ...
                        <TableCell className="h-12 py-1">
                          {isEditing ? (
                            ...
                            >
                              {entry.profit ? formatCurrency(entry.profit) : ""}
                            </span>
                          )}
                        </TableCell>
                        <TableCell className="h-12 py-1">
                          {isEditing ? (
                            ...
                            >
                              {entry.brokerage
                                ? formatCurrency(entry.brokerage)
                                : ""}
                            </span>
                          )}
                        </TableCell>
                        <TableCell className="h-12 py-1">
                          {calculated.brokPercent}
                        </TableCell>
                        <TableCell
                          className={cn(
                        ...
                        <div className="flex flex-col space-y-2 group">
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-semibold text-muted-foreground">
                              ...
                              {format(weekDays[weekDays.length - 1], "MMM dd")})
                            </span>
                            <Button
                              variant="ghost"
                              size="icon"
                              ...
                              onClick={() =>
                                toggleNoteEdit(weekKey, weekDays[0])
                              }
                            >
                              {editingNote === weekKey ? (
                                isNoteChanged(weekKey) ? (
                                  <Check className="h-3 w-3" />
                                ) : (
                                  <X className="h-3 w-3" />
                                )
